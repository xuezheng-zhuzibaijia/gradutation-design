...
//Deletion
/*
 *get_neighbor_index--find the index of pointer left to n
...
/*
 *after deletion,the num keys of n is too few,so has to collapse with his neighbor
...
node_pointer tree_delete(node_pointer root,int key){
    ...
    if(TOID_IS_NULL(leaf)){
        return root;
    }return delete_entry(root,leaf,key);
}
